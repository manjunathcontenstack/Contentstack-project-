(function(){
  ...
  async function fetchJSON(url){
    ...
    const full = `${API_HOST}/v3${url}${sep}locale=${encodeURIComponent(LOCALE)}&ts=${Date.now()}`;
    ...
    try {
      ...
      sections.forEach(blk => {
        if (blk.feature_section) {
          ...
          const img = host.querySelector('.cap-media img'); if (img && (fs.image?.url || fs.image?.file?.url)) img.src = cacheBust(fs.image.url || fs.image.file.url);
  ...
  function applyCompany(entry){
    try {
      ...
      if (ov) {
        ...
        if (img && src) img.src = cacheBust(src);
      }
      // Mission
      ...
      if (ld && Array.isArray(ld.leaders)) {
        ...
        if (grid) {
          ...
          ld.leaders.forEach(l => {
            ...
            img.src = src ? cacheBust(src) : 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=1200&q=80&auto=format&fit=crop';
        ...
        if (band) {
          ...
          if (grid && Array.isArray(offices.images)) {
            ...
            offices.images.forEach((img) => {
              ...
              el.alt = 'Office';
              el.src = url ? cacheBust(url) : 'https://images.unsplash.com/photo-1521737604893-d14cc237f11d?w=1200&q=80&auto=format&fit=crop';
              grid.appendChild(el);
            });
          }
        }
      ...
      if (hero) {
        ...
        if (gal && Array.isArray(hero.gallery) && hero.gallery.length) {
          ...
          hero.gallery.forEach(img => {
            ...
            const el = document.createElement('img');
            el.alt = 'Team';
            el.src = url ? cacheBust(url) : 'https://images.unsplash.com/photo-1544005313-94ddf0286df2?w=1200&q=80&auto=format&fit=crop';
            gal.appendChild(el);
          });
        }
      }
  ...
  async function init(){
    ...
    try {
      if (path.endsWith('/index.html') || path === '/' || path === '') {
      ...
      } else if (path.endsWith('/platform.html') || path.endsWith('/platform')) {
        ...
        if (partners.length) {
          ...
          partners.forEach(p => {
            ...
            img.src = url ? cacheBust(url) : 'https://via.placeholder.com/64';
      ...
      } else if (path.endsWith('/blog.html') || path.endsWith('/blog')) {
          ...
          if (main && item) {
            ...
            main.innerHTML = `
              <article class="doc" style="max-width:800px;margin:0 auto;">
                ...
                <div style="margin-top:18px"><a class="btn ghost" href="/blog.html">← Back to blog</a></div>
              </article>`;
          }
        } else {
          ...
          if (items.length) {
            ...
            if (grid) {
              ...
              items.forEach(b => {
                ...
                const url = f?.url || (f?.uid ? assetMap[f.uid] : null);
                img.src = url ? cacheBust(url) : 'https://images.unsplash.com/photo-1518770660439-4636190af475?w=800&q=60&auto=format&fit=crop';
      ...
      } else if (path.endsWith('/docs.html') || path.endsWith('/docs')) {
        ...
        if (items.length) {
          ...
          if (grid) {
            ...
            items.forEach(app => {
              ...
              const f = app.logo;
              const url = f?.url || (f?.uid ? assetMap[f.uid] : null);
              img.src = url ? cacheBust(url) : 'https://images.unsplash.com/photo-1545239351-1141bd82e8a6?w=256&q=80&auto=format&fit=crop';
        ...
        if (items.length) {
          ...
          if (grid) {
            ...
            items.forEach(ac => {
              ...
              const url = f?.url || (f?.uid ? assetMap[f.uid] : null);
              img.src = url ? cacheBust(url) : 'https://images.unsplash.com/photo-1521737604893-d14cc237f11d?w=1200&q=80&auto=format&fit=crop';
      ...
      } else if (path.endsWith('/company.html') || path.endsWith('/company')) {
